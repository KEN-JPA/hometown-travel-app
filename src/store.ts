...
interface TravelStore {
  ...
  permanentlyDeleteTrip: (id: string) => void;
  ...
  persist(
    (set) => ({
      ...
      deleteTrip: (id) => set((state) => {
        ...
        return {
          trips: newTrips,
          deletedTrips: tripToDelete ? [...(state.deletedTrips || []), tripToDelete] : (state.deletedTrips || []),
          selectedTripId: state.selectedTripId === id ? (newTrips[0]?.id || null) : state.selectedTripId
        };
      }),

      restoreTrip: (id) => set((state) => {
        const tripToRestore = (state.deletedTrips || []).find(t => t.id === id);
        if (!tripToRestore) return state;
        const newDeletedTrips = (state.deletedTrips || []).filter(t => t.id !== id);
        return {
          trips: [...state.trips, tripToRestore],
          deletedTrips: newDeletedTrips,
          selectedTripId: tripToRestore.id
        };
      }),

      permanentlyDeleteTrip: (id) => set((state) => ({
        deletedTrips: (state.deletedTrips || []).filter(t => t.id !== id)
      })),
